module FileHandler
  module Excel
    class InventoryItemHandler<Base
      ...
      def self.import(file, inventory_list)
        p file
        msg = Message.new
        book = Roo::Excelx.new file.full_path
        book.default_sheet = book.sheets.first

        validate_msg = validate_import(file, inventory_list)
        if validate_msg.result
          ...
          msg.result = true
          msg.content = "导入盘点数据成功, #{count}条记录成功改变！"
          # rescue => e
          #   puts e.backtrace
          #   msg.result = false
      ...
      def self.validate_import file, inventory_list
        ...
        p.workbook.add_worksheet(:name => "Basic Worksheet") do |sheet|
          ...
          2.upto(book.last_row) do |line|
            ...
            mssg = validate_row(row, inventory_list)
      ...
      def self.validate_row(row, inventory_list)
